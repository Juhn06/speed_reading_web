...
function renderUserDetails(data) {
    ...
    const recentRows = (data.recent_sessions || []).map((session) => `
        <tr>
            ...
            <td>${session.speed} t?/phút</td>
            <td>${session.completed ? 'Hoàn thành' : 'Chua xong'}</td>
    ...
    const recentSection = recentRows
        ? `
        <div class="detail-section-title">Phiên d?c g?n dây</div>
        ...
        `
        : `<div class="detail-empty">Chua có phiên d?c nào.</div>`;
    ...
    return `
        ...
        <div class="detail-stats">
            <div class="detail-stat">
                ...
                <div class="stat-label">Phiên d?c</div>
            </div>
            <div class="detail-stat">
            ...
            <div class="detail-stat">
                ...
                <div class="stat-label">Tài li?u</div>
            </div>
        </div>
        ${recentSection}
    `;
...
function renderDocumentDetails(data) {
    ...
    return `
        <div class="detail-grid">
            <div class="detail-item">
                <div class="detail-label">Tài li?u</div>
            ...
            <div class="detail-item">
                <div class="detail-label">Ngày upload</div>
                <div class="detail-value">${escapeHtml(data.created_at)}</div>
            </div>
        </div>
        <div class="detail-stats">
...
async function deleteUser(userId, username) {
    if (!confirm(`B?n ch?c ch?n mu?n xóa user "${username}"?\n\nT?t c? d? li?u liên quan s? b? xóa vinh vi?n!`)) {
        return;
    }

    try {
        showLoading('Ðang xóa user...');
        ...
        if (data.success) {
            ...
            showAlert(data.error || 'Có l?i x?y ra!', 'danger');
        }
    } catch (error) {
        hideLoading();
        showAlert('L?i xóa user: ' + error.message, 'danger');
    }
}
...
function viewUserDetails(userId) {
    ...
    if (!modalBody) {
        showAlert('Không th? m? chi ti?t ngu?i dùng', 'danger');
        return;
    }

    openUserModal();
    modalBody.innerHTML = '<div class="modal-loading">Ðang t?i d? li?u...</div>';

    fetch(`/admin/user/${userId}/details`)
        .then((response) => {
            if (!response.ok) {
                throw new Error('Không th? t?i d? li?u');
            }
        ...
        .catch((error) => {
            modalBody.innerHTML = `<div class="detail-empty">${escapeHtml(error.message || 'Có l?i x?y ra')}</div>`;
        });
}
...
function viewDocument(docId) {
    ...
    if (!modalBody) {
        showAlert('Không th? m? chi ti?t tài li?u', 'danger');
        return;
    }
...
async function deleteDocument(docId, filename) {
    if (!confirm(`B?n ch?c ch?n mu?n xóa tài li?u "${filename}"?`)) {
        return;
    }
    ...
    try {
        ...
        showAlert('L?i xóa tài li?u: ' + error.message, 'error');
    }
}
