...
def dashboard():
    ...
    featured_doc = Document.query.filter_by(user_id=current_user.id, is_starred=True) \
    ...
    featured_progress = 0
    if featured_doc:
        featured_session = ReadingSession.query.filter_by(
            user_id=current_user.id,
            document_id=featured_doc.id
        ).order_by(ReadingSession.created_at.desc()).first()

        if featured_session:
            featured_progress = featured_session.get_completion_rate()

    return render_template('reading/dashboard.html',
                           stats=stats,
                           recent_sessions=recent_sessions,
                           recent_docs=recent_docs,
                           featured_doc=featured_doc,
...
@user_bp.route('/document/<int:doc_id>/star', methods=['POST'])
@login_required
def toggle_star(doc_id):
    ...
    if doc.is_starred:
        ...
        Document.query.filter_by(user_id=current_user.id, is_starred=True).update(
    ...
    return jsonify({'success': True, 'starred': doc.is_starred})
...
def _dedupe_documents_for_user(user_id):
    ...
    for doc in docs:
        ...
        if key in keep:
            if doc.is_starred and not keep[key].is_starred:
                keep[key].is_starred = True
                changed += 1
